# StackVertex Developer's Encyclopedia - Teil 2
...
### 1.1 StackVertex Infrastructure (Platform selbst)

StackVertex nutzt Terraform für die **eigene** Platform-Infrastruktur (nicht zu verwechseln mit den Terraform Files die für User-Architectures generiert werden).
...
terraform {
  ...
  backend "s3" {
    bucket         = "stackvertex-terraform-state-dev"
    key            = "dev/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
    dynamodb_table = "stackvertex-terraform-locks"  # State locking
  }
}
...
module "storage" {
  ...
  # DynamoDB Main Table
  dynamodb_table_name = "stackvertex-dev-main"
  dynamodb_billing_mode = "PAY_PER_REQUEST"  # On-Demand für dev
  
  # S3 Buckets
  s3_large_items_bucket = "stackvertex-dev-large-items"
  s3_terraform_state_bucket = "stackvertex-terraform-state-dev"
...
module "compute" {
  ...
  lambda_function_name = "stackvertex-backend-dev"
...
module "api_gateway" {
  ...
  # domain_name = "api-dev.stackvertex.com"
...
TerraformGeneratorV2.generate(architecture_json)
    ↓
Terraform Files (.tf) written to /tmp/stackvertex/deployments/{deployment_id}/
    ↓
...
# ============================================================================
# Generated by StackVertex Infrastructure Designer
  ...
  build:
    ...
    steps:
      ...
      - name: Extract metadata
        ...
        with:
          images: ${{ secrets.DOCKER_USERNAME }}/stackvertex-backend
          tags: |
    ...
    steps:
      ...
      - name: Deploy to Lambda
        run: |
          ...
          aws lambda update-function-code \
            --function-name stackvertex-backend-prod \
            --image-uri ${{ secrets.DOCKER_USERNAME }}/stackvertex-backend:latest
      ...
      - name: Health check
        run: |
          ...
          # Check API health
          curl -f https://api.stackvertex.com/health || exit 1
...
jobs:
  ...
  deploy:
    ...
    steps:
      ...
      - name: Sync to S3
        run: |
          aws s3 sync dist/ s3://stackvertex-frontend-prod/ \
            --delete \
